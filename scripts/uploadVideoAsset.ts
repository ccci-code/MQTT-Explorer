...
async function tagUrl(tag: string): Promise<string | undefined> {
  const response = await axios.get(
    `https://api.github.com/repos/ccci-code/mqtt-explorer/releases?access_token=${githubToken}`
  )
...
async function createDraft(tag: string) {
  ...
  const response = await axios({
    method: 'post',
    url: `https://api.github.com/repos/ccci-code/mqtt-explorer/releases?access_token=${githubToken}`,
    data: {
      tag_name: tag,
      name: tag.slice(1),
      draft: true,
